...
export default function GeneralInbox({ userId, onMessageClick, detailSidebarOpen, setDetailSidebarOpen }) {
  ...
  const fetchMessages = async (receiverId) => {
    try {
      const res = await axios.get(`${Chat_key}/api/messages/${userId}/${receiverId}`);
  ...
  const updateMessageStatus = async (messageId, status) => {
    try {
      await axios.put(`${Chat_key}/api/messages/status/${messageId}`, { status });
  ...
  const handleSendMessage = async () => {
    ...
    try {
      ...
      socket.emit("sendMessage", { ...savedMessage, room: selectedMessage.user });
socket.emit("updateMessageStatus", { 
  messageId: savedMessage._id, 
  status: "delivered" 
});
      setTimeout(() => {
        updateMessageStatus(savedMessage._id, "delivered");
      }, 1000);
    ...
    <>
      ...
      >         
       ...
       <div className="p-4 border-b border-[#1E1E1E] flex items-center">
            <button onClick={() => setDetailSidebarOpen(false)} className="p-2 rounded-full hover:bg-gray-800 mr-2">
              <ArrowLeft size={20} />
            </button>
          ...
          <div className="p-4 border-t border-[#1E1E1E]">
            <div className="relative flex items-center">
              ...
              />
              <button className="absolute right-3 bg-black p-2 rounded-full" onClick={handleSendMessage}>
                ➤
              </button>
            </div>
          </div>
