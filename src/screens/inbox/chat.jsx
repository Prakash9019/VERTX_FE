...
const Chat = ({ user, recipient }) => {
    ...
    const fetchMessages = async () => {
      try {
        const res = await axios.get(
          `http://localhost:5001/api/messages/${user._id}/${recipient._id}`
        );
  ...
  useEffect(() => {
    const checkConnection = async () => {
      try {
        const res = await axios.get(
          `http://localhost:5001/api/connections/status/${user._id}/${recipient._id}`
        );
  ...
  return (
    <div style={styles.chatContainer}>
      <h3>Chat with {recipient.firstName}</h3>
      ...
      {isConnected ? (
        <>
          <p>You are now connected with {recipient.firstName}. Start chatting!</p>
            ...
            {messages.length > 0 ? (
              messages.map((msg, index) => (
                ...
                >
                  <div>
                    <strong>
                      {msg.senderId === user._id ? "Me" : recipient.firstName}:
                    </strong>{" "}
                    {msg.message}
                  </div>
