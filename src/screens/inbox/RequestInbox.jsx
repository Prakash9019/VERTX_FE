...
const RequestInbox = ({ userId }) => {
  ...
  return (
    <div className="flex flex-col md:flex-row h-full -mt-7">
      <div className={`flex-grow bg-black p-2 overflow-y-auto md:w-2/3 ${detailSidebarOpen ? "hidden md:block" : "block"}`} style={mainContentStyles}> 
        {requests?.length > 0 ? (
              ...
              <div className="flex-1 text-right">
                <span className="text-xs text-white">{message?.createdAt}</span>
                    ...
                    <div className="flex flex-col items-center justify-center h-full">
                        <p className="text-gray-400 text-center">No requests found. All requests will be appear here.</p>
      ...
      {detailSidebarOpen && selectedMessage && (
        <div className="fixed top-0 right-0 h-full w-full md:w-1/3 bg-black border-l border-gray-800 z-50 transform transition-transform duration-300 ease-in-out">
