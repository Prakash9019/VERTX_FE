        ...
        <>
          ...
          {showProfilePopup && localStorage.getItem("token") && (
            <>
              ...
              <div className="fixed inset-0 bg-black/80 z-30" onClick={() => setShowProfilePopup(false)}></div>
              {/* Popup menu */}
...
export function Sidebar({ sidebarOpen, setSidebarOpen }) {
  ...
  return (
    <>
      <div
        className={`flex flex-col h-full bg-black transition-all duration-700 ease-in-out ${sidebarOpen ? "w-64" : "w-24"} fixed left-0 top-0 bottom-0 z-10`}
      >
        {/* Logo */}
        <div className="p-5 pb-2 flex items-center">
          <div className={`transition-all duration-700 ease-in-out ${sidebarOpen ? "ml-3" : "-ml-2"} mt-4 pt-3 text-white`}>
            <img src={logo || "/placeholder.svg"} alt="image" className={`transition-all duration-700 ease-in-out w-12 ${sidebarOpen ? "h-10" : "h-12"}`} />
          </div>
          {sidebarOpen && (
            ...
            <path d="M6.72 0.799999H8.656V8.176L5.008 12H3.584L0.944 8.096V0.799999H2.88V7.728L4.416 10.016L6.72 7.6V0.799999ZM11.8498 0.799999H18.0898V2.608H12.5858L12.2978 2.928V5.44H15.9938V7.248H12.2978V9.488L12.7618 10.192H18.0898V12H11.8018L10.3618 9.856V2.32L11.8498 0.799999ZM20.1155 0.799999H25.9875L27.8595 3.536V5.408L26.3875 6.928L27.8595 9.088V12H25.9235V9.424L24.4675 7.296H22.8995L22.0515 6.768V12H20.1155V0.799999ZM22.0515 2.608V5.488H25.2995L25.9235 4.848V3.92L25.0115 2.608H22.0515ZM29.4053 0.799999H37.7573V2.608H33.5013L34.5573 3.552V12H32.6213V4L32.1573 2.608H29.4053V0.799999ZM44.791 0.799999H46.727V4.208L45.207 5.76L46.727 7.984V12H44.791V8.352L43.975 7.216H43.239L41.559 8.976V12H39.623V8.368L41.143 6.816L39.623 4.592V0.799999H41.559V4.224L42.343 5.408H43.111L44.791 3.648V0.799999Z" fill="white"/>
              ...
              <button onClick={toggleSidebar} className="ml-2 mb-4 bg-black p-1 rounded-full transition-transform duration-300 ease-in-out hover:scale-110">
              <svg
                  height="22"
                  viewBox="0 0 13 13"
                  fill="none"
...
<path d="M379.86 417.556C339.877 449.371 289.26 468.37 234.186 468.37C179.112 468.37 128.496 449.371 88.5095 417.556C117.79 374.542 172.07 345.654 234.186 345.654C296.302 345.654 350.587 374.535 379.86 417.556Z" fill="#EEEEEE" fill-opacity="0.933333"/>
          ...
          {showDesktopProfilePopup && localStorage.getItem("token") && (
            <>
              ...
              <div className={`absolute ${sidebarOpen ? 'bottom-[130px] right-[-210px]' : 'bottom-[100px] right-[-220px]'} z-50 w-64 bg-black rounded-lg shadow-lg border border-[#333] p-4 animate-fadeIn`}>
                <div className="py-2">
                  {[
                    { icon: "account_circle", label: "Overview", action: handleOverviewClick },
                    { icon: "settings", label: "Settings" },
...
export function MainContent({ sidebarOpen, children }) {
  ...
  return (
    <div className={`flex-1 transition-all duration-700 ease-in-out ${isMobile ? "pt-16 pb-16" : sidebarOpen ? "ml-64" : "ml-24"} ${!isMobile && "pt-9 pr-6 pb-6"}`}>
...
function NavItem({ icon, label, expanded, active, name, onClick }) {
  return (
    <button
      onClick={onClick}
      className={`flex items-center py-4 px-4 relative ${active === name ? "text-white" : "text-gray-400"} hover:text-white cursor-pointer transition-colors duration-200`}
    >
      <div className="w-6 h-6">{icon}</div>
      {expanded && <span className="ml-4 text-lg transition-opacity duration-700 ease-in-out">{label}</span>}
      {active === name && !expanded && <div className="absolute right-0 w-1 h-8 bg-white rounded-md transition-all duration-700 ease-in-out"></div>}
    </button>
  )
}
...
export function NavIconFooter({ icon, label, active = false }) {
  return (
    <div className={`flex flex-col items-center ${active ? "text-white" : "text-gray-400"} transition-colors duration-200`}>
      <div className="w-6 h-6">{icon}</div>
