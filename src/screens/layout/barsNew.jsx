  ...
  return (
    <div className={`flex flex-col h-full bg-black transition-all duration-300 ${sidebarOpen ? "w-64" : "w-24"} fixed left-0 top-0 bottom-0 z-10`}>
      ...
      <div className="ml-3 flex flex-col flex-grow mt-10">
        <NavItem icon={<Search />} label="Explore" expanded={sidebarOpen} />
        <NavItem icon={<Target />} label="Outreach" expanded={sidebarOpen} active />
        <NavItem icon={<Users />} label="Engage" expanded={sidebarOpen} />
        <NavItem icon={<Grid />} label="Resources" expanded={sidebarOpen} />
      </div>
...
function NavItem({ icon, label, expanded, active = false }) {
  return (
    <div className={`flex items-center py-4 px-4 relative ${active ? "text-white" : "text-gray-400"} hover:text-white cursor-pointer`}>
